...
{
    public partial class Tareas_Directores_Nacionales : System.Web.UI.Page
    {
        ...
        protected string Grid(int op)
        {
            ...
            string CadenaSQL = "SELECT ad.Id_PoAnual,ad.IdMensaje,ad.Id_Region,ad.Id_Subregion,"+
                               "UPPER(pc.Descripcion_POA + ' ' + CAST(pc.Anio_Correspondiente AS varchar)) POA,mt.DescripcionMensaje Etapa,"+
                               "CONVERT(VARCHAR(12), ad.FechaDeAsignacion, 103) FechaDeAsignacion,CONVERT(VARCHAR(12), ad.FechaDeEntrega, 103) FechaDeEntrega,"+
                               "ad.EstadoDeAsignacion,r.Nombre_Region,(SELECT Instrucciones FROM MantenimientoPoaNacional WHERE Id_PoAnual = ad.Id_PoAnual) Instrucciones,"+
                               "ad.Asignado,ad.TipoAsignacion,Isnull(ad.NoReprogramacion, 0) NoReprogramacion "+
                               "FROM AsignacionDirectoresNacional ad "+
                               "INNER JOIN Poas_Creados pc ON pc.Id_PoAnual = ad.Id_PoAnual "+
                               "INNER JOIN  Mensaje_Tarea mt ON mt.Id_Mensaje = ad.IdMensaje "+
                               "INNER JOIN Region r ON r.Id_Region = ad.Id_Region WHERE ad.EstadoDeAsignacion = 1 AND Asignado = 0";

            if (op == 1)
            {
                CadenaSQL += "AND ad.Id_Region =" + Users.id_region;
            }
            return CadenaSQL;
        }
        protected void GridTareaP_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            ...
            if ((PermisosUsuario.Verifica_Permisos(PermisosPermitidos, 52).Permiso != true) && (us.Id_Tipoperfil == 1))
            {
                Op = 0;
            }
            else
        ...
        protected void IniciarTarea_Click(object sender, EventArgs e)
        {
            ...
            {
                if (x.Inicializar_TareaPoaDepartamentoPaque(dt, ref er))
                {
