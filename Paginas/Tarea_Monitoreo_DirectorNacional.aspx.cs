...
{
    public partial class Tarea_Monitoreo_DirectorNacional : System.Web.UI.Page
    {
        ...
        protected string Grid(int op)
        {
            ...
            string CadenaSQL = "SELECT arm.Id_PoAnual,arm.IdMensaje,arm.Id_Region,arm.Id_Subregion,UPPER('POA NACIONAL ' + CAST(pc.Anio_Correspondiente AS varchar)) POA," +
                               ...
                               "FROM AsignacionDirectoresNacionalMonitoreo arm INNER JOIN Poas_Creados pc ON pc.Id_PoAnual = arm.Id_PoAnual INNER JOIN  Mensaje_Tarea mt ON mt.Id_Mensaje = arm.IdMensaje " +
        ...
        protected void GridTareaP_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            ...
            if ((PermisosUsuario.Verifica_Permisos(PermisosPermitidos, 91).Permiso != true) && (us.Id_Tipoperfil == 1))
            {
                Op = 0;
            }
            else
        ...
        protected void IniciarTarea_Click(object sender, EventArgs e)
        {
            ...
            {
                if (x.Inicializar_TareaPoaDepartamentoPaque2(dt, ref er))
                {
