...
{
    public partial class X_IncentivoForestales : System.Web.UI.Page
    {
        ...
        private void CboTipoIncentivo_TextChanged(object sender, EventArgs e)
        {
            cboIncetivos.ClearSelection();
            lblIncentivo.Text = procesos.StrNULLCombo(CboTipoIncentivo);
            string CadenaSQl = "SELECT Id_Incentivo Id,Descripcion FROM IncentivoForestal WHERE Estado = 1 AND Id_tipoIncentivo = "+ procesos.IntNULLCombo(CboTipoIncentivo).ToString();
            procesos.LLenarComboT (cboIncetivos, CadenaSQl, "Descripcion", "Id", true);
        }
        ...
        protected void Seleccionar_Productos(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Select")
            {
                ...
                ProductosIncent.Visible = false;
                Guardar.Visible = false;

                if ((Producto == 85) || (Producto == 86))
                {
                    Guardar.Visible = true;
                    txtAreaIncentivo.Visible = false;
                    txtAreaIncentivo.Text = string.Empty;
                    Label15.Visible = false;
                    ProductosIncent.Visible = true;
                    CadenaSQl = "SELECT Id_TipoIncentivo Id,Descripcion FROM Tipo_Incentivo WHERE Estado = 1;";
                    procesos.LLenarComboT(CboTipoIncentivo, CadenaSQl, "Descripcion", "Id", true);
                    CadenaSQl = "SELECT Id_fase Id,Descripcion FROM FaseProyecto WHERE Estado = 1;";
                    procesos.LLenarComboT(CboFaseProyecto, CadenaSQl, "Descripcion", "Id", true);
                }
                if ((Producto == 84) || (Producto == 87) || (Producto == 88) || 
                    (Producto == 89) || (Producto == 83) || (Producto == 90)|| (Producto == 275)|| (Producto == 276)|| (Producto == 277)|| (Producto == 313)) 
                {
                    Guardar.Visible = true;
                }
                RecargarGrid();
        ...
        public Validar_Data Verificar_VaciosSalida()
        {
            ...
            if ((DMI.Id_ProductoVerificable == 90) || (DMI.Id_ProductoVerificable == 83))
            {
        ...
        protected void Ini() 
        {
            ...
            txtAreaIncentivo.Text = string.Empty;
            CboTipoIncentivo.ClearSelection();
            cboIncetivos.ClearSelection();
            CboFaseProyecto.ClearSelection();
            txtFecha.Clear();
        }
        ...
        {
            DatosMonitoreoIngresoMetas DMI = (DatosMonitoreoIngresoMetas)Session["DatosMonitoreoIngresoMetasENVIO"];
            string CadenaSql = "Sp_obtener_data_GridProductosMonitoreo " + 6 + "," + DMI.Id_PoAnual + "," + DMI.Id_Subregion + "," + DMI.IdComponente
        ...
        protected void GrdIngresoEncabezado_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridDataItem)
            {
                ...
                GrdIngresoEncabezado.MasterTableView.GetColumn("BotonD").Display = VPC.ShowHideProductoIncentivoForestalM(Producto, 1);
                GrdIngresoEncabezado.MasterTableView.GetColumn("BotonC").Display = VPC.ShowHideProductoIncentivoForestalM(Producto, 1);
                /*validar Valores 0*/
        ...
        protected void Inicializacion_Objetos()
        {
            ...
            CboTipoIncentivo.TextChanged += new EventHandler(CboTipoIncentivo_TextChanged);
        ...
        protected void Guardar_Click(object sender, EventArgs e)
        {
            ...
            EncabezadoIncentivosForestales Generico = new EncabezadoIncentivosForestales();
            ManejoInformacionMonitoreo Mim = new ManejoInformacionMonitoreo();           
            Mensajes_Error_BDD er = new Mensajes_Error_BDD();
            Medios DA;           
            Validar_Data v;
            ...
            Generico.NumeroExpediente = txtExpediente.Text.ToString();   
            Generico.Area = procesos.DecimalRadNumericTextBox(txtAreaIncentivo);
            Generico.Id_TipoIncentivo = procesos.IntNULLCombo(CboTipoIncentivo);

            //Generico.Id_Incentivo = cboIncetivos.SelectedValue;
            Generico.Id_Incentivo = procesos.IntNULLCombo(cboIncetivos);
            Generico.Id_fase = procesos.IntNULLCombo(CboFaseProyecto);
            ...
            v = Validar.Campos_Obligatorio_IncentivosF(Generico, RadDocumentoVerificacion, DMI,Mantenimiento);
            if (v.Verificar)
            {
                MensajePantalla(v.Mensaje);
            }
            else
            {
                ...
                XmlDocument DescripcionProducto = Mim.DetalleXMLProductoEncabezadoIncetivoForestal(Generico);

                if (Mim.GuardarItemEncabezado(5, DMI, DescripcionProducto, ref er))
                {
        ...
        protected void Eliminar_Items(object source, GridCommandEventArgs e)
        {
            ...
            {
                if (Mim.Eliminacion_Producto(7, edm, ref er))
                {
                    RecargarGrid();
                }
                else
        ...
        protected void Seleccionar_Items(object sender, GridCommandEventArgs e)
        {
            ...
            if (Llave == 0)
            {
                ...
                {
                    x1.Visible = true;
                    lblNoExpediente.Text = item.GetDataKeyValue("NumeroExpediente").ToString();
                    LblArea.Text = item.GetDataKeyValue("Area").ToString();
                    lblTipoIncentivo.Text = item.GetDataKeyValue("DescripcionTipoIncentivo").ToString();
                    lblModalidad.Text = item.GetDataKeyValue("DescripcionModalidad").ToString();
                    lblfaseProyecto.Text = item.GetDataKeyValue("DescripcionFase").ToString();
                    OpenWinwdows(VerDatosExtra, "520", "580", "Key", "Información Adicional del Ingresos");
                }
                else
                {
                    OpenWinwdows(VerDatosExtra, "520", "350", "Key", "Información Adicional del Ingresos");
                }
            }
        }
