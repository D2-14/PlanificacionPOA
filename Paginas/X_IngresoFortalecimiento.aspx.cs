...
{
    public partial class X_IngresoFortalecimiento : System.Web.UI.Page
    {
        ...
        protected void Seleccionar_Productos(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Select")
            {
                ...
                puestoEjecuta.Visible = false;
                TblPrimero.Visible = false;
                tblSegundo.Visible = false;
                tblTercero.Visible = false;
                tbl2.Visible = false;
                tbl2_1.Visible = false;
                Guardar.Visible = false;
                procesos.LLenarComboT(CboPuestoEjecuta, "SELECT Id_Puesto_Actvidad Id,Descripcion FROM Puesto_Ejecuta_Monitoreo WHERE Estado = 1;", "Descripcion", "Id", true);
                procesos.LLenarComboT(CboActor, "SELECT Id_Actores Id,Descripcion FROM Actores_Monitoreo WHERE Estado = 1;", "Descripcion", "Id", true);

                //Validar objetos visibles
                if ((DMI.Id_ProductoVerificable == 68) || (DMI.Id_ProductoVerificable == 74) || (DMI.Id_ProductoVerificable == 75) || (DMI.Id_ProductoVerificable == 81)
                     || (DMI.Id_ProductoVerificable == 82)|| (DMI.Id_ProductoVerificable == 279)|| (DMI.Id_ProductoVerificable == 278))
                     
                {
                    puestoEjecuta.Visible = true;
                    Guardar.Visible = true;
                }
                if ((DMI.Id_ProductoVerificable == 71) || (DMI.Id_ProductoVerificable == 76) || (DMI.Id_ProductoVerificable == 230))
                {
                    puestoEjecuta.Visible = true;
                    TblPrimero.Visible = true;
                    Guardar.Visible = true;
                }
                if (DMI.Id_ProductoVerificable == 72)
                {
                    tbl2.Visible = true;
                    puestoEjecuta.Visible = true;                                                          
                    tbl2_1.Visible = true;
                ...
                if (DMI.Id_ProductoVerificable == 74)
                {
                    tbl2.Visible = false;
                    V3.Visible = false;
                ...
                if (DMI.Id_ProductoVerificable == 79)
                {
                    puestoEjecuta.Visible = true;
                    tbl2_1.Visible = true;
                    Guardar.Visible = true;
                }
                if ((DMI.Id_ProductoVerificable == 78)) 
                {
                    puestoEjecuta.Visible = true;
                    tblTercero.Visible = true;
                    tblSegundo.Visible = true;
                    Guardar.Visible = true;
                }

                if ((DMI.Id_ProductoVerificable == 70) || (DMI.Id_ProductoVerificable == 279)|| (DMI.Id_ProductoVerificable == 80)|| (DMI.Id_ProductoVerificable == 73)|| (DMI.Id_ProductoVerificable == 77) || (DMI.Id_ProductoVerificable == 69)) 
                {
                    puestoEjecuta.Visible = false;
                    tblTercero.Visible = false;
                    tblSegundo.Visible = false;
        ...
        public Validar_Data Verificar_VaciosSalida()
        {
            ...
            if ((Producto == 69) || (Producto == 70) || (Producto == 73) || (Producto == 77)
                    || (Producto == 78) || (Producto == 80))
            {
                if (Mim.Validad_Llenado_detalle(1, DMI) == false)
                {
                    v.Verificar = true; v.Mensaje += "Debe de Ingresar Información Tipo de actor.<br/>";
                }
        ...
        {
            DatosMonitoreoIngresoMetas DMI = (DatosMonitoreoIngresoMetas)Session["DatosMonitoreoIngresoMetasENVIO"];
            string CadenaSql = "Sp_obtener_data_GridProductosMonitoreo " + 3 + "," + DMI.Id_PoAnual + "," + DMI.Id_Subregion + "," + DMI.IdComponente
        ...
        protected void GrdIngresoEncabezado_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridDataItem)
            {
                ...
                GrdIngresoEncabezado.MasterTableView.GetColumn("BotonD").Display = VPC.ShowHideProductoForta(Producto, 1);
                GrdIngresoEncabezado.MasterTableView.GetColumn("BotonC").Display = VPC.ShowHideProductoForta(Producto, 1);
                /*validar Valores 0*/
        ...
        protected void Inicializacion_Objetos()
        {
            ...
            /*Extra*/
            CboActor.TextChanged += new EventHandler(CboActor_TextChanged);
        ...
        {           
            CboPuestoEjecuta.ClearSelection();
            puestoEjecuta.Visible = false;
            TblPrimero.Visible = false;
            tblSegundo.Visible = false;
            tbl2.Visible = false;
        ...
        protected void Limpiar() 
        {
            ...
            txtFecha.Clear();
            CboPuestoEjecuta.ClearSelection();
            ttxNombreActor.Text = string.Empty;
            txttemaAtendido.Text = string.Empty;
            txtAcciones.Text = string.Empty;
            txtComunidadDireccion.Text = string.Empty;
            txtUbicacionDireccion.Text = string.Empty; 
            txtnombreDocumento.Text = string.Empty; 
            txtAnioVigencia.Text = string.Empty;
            CboActor.ClearSelection();
            CboTipoActor.ClearSelection(); 
        }
        protected void Guardar_Click(object sender, EventArgs e)
        {
            ValidarCamposObligatoriosMonitoreo Validar = new ValidarCamposObligatoriosMonitoreo();
            EncabezadoFortalecimiento Generico = new EncabezadoFortalecimiento();
            ManejoInformacionMonitoreo Mim = new ManejoInformacionMonitoreo();
            Mensajes_Error_BDD er = new Mensajes_Error_BDD();
            Medios DA;
            Validar_Data v;
            ...
            /*extra*/
            Generico.Id_Puesto_Actvidad = procesos.IntNULLCombo(CboPuestoEjecuta);
            Generico.NombreActor = ttxNombreActor.Text.ToString();
            Generico.TemaAtendido = txttemaAtendido.Text.ToString();
            Generico.AccionesSeguimiento = txtAcciones.Text.ToString();
            Generico.ComunidadOrganizacion = txtComunidadDireccion.Text.ToString();
            Generico.UbicacionOrganizacion = txtUbicacionDireccion.Text.ToString();
            Generico.NombreDocumento = txtnombreDocumento.Text.ToString();
            Generico.AnioVigenciaPolitica = procesos.STRRadNumericTextBox(txtAnioVigencia);
            Generico.TipoActor = procesos.IntNULLCombo(CboActor);
            Generico.Actores = procesos.IntNULLCombo(CboTipoActor);
            ...
            v = Validar.Campos_Obligatorio_Fortalecimiento(Generico, RadDocumentoVerificacion, Mantenimiento);
            if (v.Verificar)
            {
                MensajePantalla(v.Mensaje);
            }
            else
            {
                ...
                XmlDocument DescripcionProducto = Mim.DetalleXMLProductoFortalecimiento(Generico);

                if (Mim.GuardarItemEncabezado(3, DMI, DescripcionProducto, ref er))
                {
        ...
        protected void Eliminar_Items(object source, GridCommandEventArgs e)
        {
            ...
            {
                if (Mim.Eliminacion_Producto(3, edm, ref er))
                {
                    RecargarGrid();
                }
                else
        ...
        protected void Seleccionar_Items(object sender, GridCommandEventArgs e)
        {
            ...
            if (Llave == 0)
            {
                ...
                V5.Visible = false;
                V10.Visible = false;
                LObserva.Text = item.GetDataKeyValue("Observaciones").ToString();
                lblActividad.Text = item.GetDataKeyValue("Actividad").ToString();
                int Producto = Convert.ToInt32(item.GetDataKeyValue("Id_ProductoVerificable").ToString());
                ...
                if ((Producto == 71) || (Producto == 76) || (Producto == 230))
                {
                    V1.Visible = true;
                    V2.Visible = true;
                    lblUbicacion.Text = item.GetDataKeyValue("UbicacionOrganizacion").ToString();
                    lblComunidadorganizacion.Text = item.GetDataKeyValue("ComunidadOrganizacion").ToString();
                    OpenWinwdows(VerDatosExtra, "520", "500", "Key", "Información Adicional del Ingresos");
                }
                if (Producto == 72)
                {
                    V1.Visible = true;
                    V3.Visible = true;
                    V4.Visible = true;
                    lbldocumento.Text = item.GetDataKeyValue("NombreDocumento").ToString();
                    lblanio.Text = item.GetDataKeyValue("AnioVigenciaPolitica").ToString();
                    LblAcciones.Text = item.GetDataKeyValue("AccionesSeguimiento").ToString();
                ...
                if ((Producto == 69) || (Producto == 70) || (Producto == 73) || (Producto == 77)|| (Producto == 78) || (Producto == 80))
                {
                    ...
                    V5.Visible = true;
                    V10.Visible = true;
                    lblTipoActor.Text = item.GetDataKeyValue("Actores").ToString();
                    lbltituloActor.Text = item.GetDataKeyValue("Actores").ToString();
                    lblActorT.Text = item.GetDataKeyValue("TipoActores").ToString();
                    LblActor.Text = item.GetDataKeyValue("NombreActor").ToString();
                    lbltemaA.Text = item.GetDataKeyValue("TemaAtendido").ToString();
                    OpenWinwdows(VerDatosExtra, "520", "560", "Key", "Información Adicional del Ingresos");
                }                
            }
        }
        ...
        /**/
        private void CboActor_TextChanged(object sender, EventArgs e)
        {
            CboTipoActor.ClearSelection();
            Label28.Text = procesos.StrNULLCombo(CboActor);
            string stringsql = "SELECT Id_TipoActor Id,Descripcion FROM Tipo_Actores_Monitoreo WHERE Estado = 1 AND Id_Actores = " + procesos.IntNULLCombo(CboActor);           
            procesos.LLenarComboT(CboTipoActor, stringsql, "Descripcion", "Id", true);
        }
