...
{
    public partial class X_IngresoRNF : System.Web.UI.Page
    {
        ...
        protected void Seleccionar_Productos(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Select")
            {
                ...
                if (Producto == 255)
                {
                    ...
                    procesos.LLenarComboT(CboDenegacion, "SELECT Id_TipoDenegacion Id,Descripcion FROM Tipo_DenegacionRNF WHERE Estado = 1;", "Descripcion", "Id", true);
        ...
        {
            DatosMonitoreoIngresoMetas DMI = (DatosMonitoreoIngresoMetas)Session["DatosMonitoreoIngresoMetasENVIO"];
            string CadenaSql = "Sp_obtener_data_GridProductosMonitoreo " + 15 + "," + DMI.Id_PoAnual + "," + DMI.Id_Subregion + "," + DMI.IdComponente
        ...
        protected void GrdIngresoEncabezado_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridDataItem)
            {
                ...
                GrdIngresoEncabezado.MasterTableView.GetColumn("BotonA").Display = VPC.ShowHideProductoRNF(Producto, 1);
                /*validar Valores 0*/
        ...
        protected void Guardar_Click(object sender, EventArgs e)
        {
            ...
            EncabezadoRNF Generico = new EncabezadoRNF();
            ManejoInformacionMonitoreo Mim = new ManejoInformacionMonitoreo();
            ...
            Generico.NoExpediente = txtNoExpediente.Text;
            Generico.CategoriaRNF = txtCategoria.Text;
            Generico.SubcategoriaRNF = txtSubcategoria.Text;
            Generico.Especificaciones = txtEspecificaciones.Text; 
            Generico.IdTipoDenegacion = procesos.IntNULLCombo(CboDenegacion);
            ...
            v = Validar.Campos_Obligatorio_RNF(Generico, RadDocumentoVerificacion,Mantenimiento);
            if (v.Verificar)
            {
            ...
            else
            {
                ...
                if ((Producto == 218) || (Producto == 219))
                {
                    ...
                    DescripcionProductoAPI = mapi.RegistroNacionalForestal(Edapi);
                }
                else
                {
                ...
                XmlDocument DescripcionProducto = Mim.DetalleXMLProductoRNF(Generico);
                if (Mim.GuardarItemEncabezadoApi(5, DMI, DescripcionProducto, DescripcionProductoAPI, ref er))
                {
                    if ((Producto != 218) || (Producto != 2019))
                    {
        ...
        protected void Eliminar_Items(object source, GridCommandEventArgs e)
        {
            ...
            {
                if (Mim.Eliminacion_Producto(16, edm, ref er))
                {
                    RecargarGrid();
                }
                else
        ...
        protected void Seleccionar_Items(object sender, GridCommandEventArgs e)
        {
            ...
            if (Llave == 0)
            {
                ...
                if(Producto == 255)
                {
                    ...
                    lblCategoria.Text = item.GetDataKeyValue("CategoriaRNF").ToString();
                    lblSubcategoria.Text = item.GetDataKeyValue("SubcategoriaRNF").ToString();
                    lblEspecificaciones.Text = item.GetDataKeyValue("Especificaciones").ToString();
                    lblDenegacion.Text = item.GetDataKeyValue("Denegacion").ToString();
                ...
                if ((Producto == 218) || (Producto == 219))
                {
                    ...
                    x1.Visible = false;
                    x2.Visible = true;
                    lblNoRegistro.Text = item.GetDataKeyValue("NoRegistro").ToString();
                    lblNoExpediente1.Text = item.GetDataKeyValue("NoExpediente").ToString();
                    lblFechaCorresponde1.Text = item.GetDataKeyValue("Fecha").ToString();
                    lblTipoRegistro.Text = item.GetDataKeyValue("TipoRegistro").ToString();
                    lblEstadoRNF.Text = item.GetDataKeyValue("EstadoRNF").ToString();
                }
