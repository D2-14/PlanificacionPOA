...
namespace PlanificacionPOA.Paginas
{
    ...
    public partial class X_IndustriayComercio : System.Web.UI.Page
    {
        ...
        private void CboTema_TextChanged(object sender, EventArgs e) 
        {
            if (procesos.IntNULLCombo(CboTema) == 7) 
            {
                Parte11.Visible = true; 
            }
            else 
            {
                Parte11.Visible = false;
            }
        }
        private void CboTipoOrganizacion_TextChanged(object sender, EventArgs e) 
        {
            if (procesos.IntNULLCombo(CboTipoOrganizacion) == 4)
            {
                Parte31.Visible = true;
            }
            else
            {
                Parte31.Visible = false;
            }
        }
        private void CboTipoEmpresa_TextChanged(object sender, EventArgs e)
        {
            if (procesos.IntNULLCombo(CboTipoOrganizacion) == 4)
            {
                Parte41.Visible = true;
            }
            else
            {
                Parte41.Visible = false;
            }
        }
        ...
        protected void Seleccionar_Productos(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Select")
            {
                ...
                //{
                //    Parte1.Visible = true;
                //    procesos.LLenarComboT(CboTema, "SELECT Id_tema Id,Descripcion FROM Tema_Industria WHERE Estado = 1;", "Descripcion", "Id", true);
                //    Guardar.Visible = true;
                //}
                //if ((Producto == 94))
                //{
                ...
                //    procesos.LLenarComboT(CboTipoOrganizacion, "SELECT Id_TipoOrganizacion Id,Descripcion FROM Tipo_Organizacion WHERE Estado = 1;", "Descripcion", "Id", true);
                //    procesos.LLenarComboT(CboTipoEmpresa, "SELECT Id_TipoEmpresa Id ,Descripcion FROM Tipo_Empresa WHERE Estado = 1;", "Descripcion", "Id", true);
                //    Guardar.Visible = true;
                //}
                if ((Producto == 97) || (Producto == 98) || 
                    (Producto == 99) || (Producto == 100)|| (Producto == 91)|| (Producto == 94) || (Producto == 102) || (Producto == 103) || (Producto == 93)||(Producto == 92)||(Producto == 95)) 
                {
                    Guardar.Visible = true;
                }
                RecargarGrid();
        ...
        public Validar_Data Verificar_VaciosSalida()
        {
            ...
            if ((DMI.Id_ProductoVerificable == 92) || (DMI.Id_ProductoVerificable == 93) || (DMI.Id_ProductoVerificable == 95) || 
                (DMI.Id_ProductoVerificable == 102) || (DMI.Id_ProductoVerificable == 103))              
            {
        ...
        protected void Ini() 
        {
            ...
            CboTema.ClearSelection();
            txtema.Text = string.Empty;
            cboTipoRegistro.ClearSelection();
            txtNumeroRegistro.Text = string.Empty;
            CboTipoOrganizacion.ClearSelection();
            txttemaOrganizacion.Text = string.Empty;
            CboTipoEmpresa.ClearSelection();
            txttemaEmpresa.Text = string.Empty;
            txttipoproductoR.Text = string.Empty;
            txtFecha.Clear();
            Parte11.Visible = false;
            Parte31.Visible = false;
            Parte41.Visible = false;
        ...
        {
            DatosMonitoreoIngresoMetas DMI = (DatosMonitoreoIngresoMetas)Session["DatosMonitoreoIngresoMetasENVIO"];
            string CadenaSql = "Sp_obtener_data_GridProductosMonitoreo " + 7 + "," + DMI.Id_PoAnual + "," + DMI.Id_Subregion + "," + DMI.IdComponente
        ...
        protected void GrdIngresoEncabezado_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            if (e.Item is GridDataItem)
            {
                ...
                GrdIngresoEncabezado.MasterTableView.GetColumn("BotonD").Display = VPC.ShowHideProductoIndustria(Producto, 1);
                GrdIngresoEncabezado.MasterTableView.GetColumn("BotonC").Display = VPC.ShowHideProductoIndustria(Producto, 1);
                /*validar Valores 0*/
        ...
        protected void Inicializacion_Objetos()
        {
            ...
            CboTema.TextChanged += new EventHandler(CboTema_TextChanged);
            CboTipoOrganizacion.TextChanged += new EventHandler(CboTipoOrganizacion_TextChanged);
            CboTipoEmpresa.TextChanged += new EventHandler(CboTipoEmpresa_TextChanged);
        ...
        protected void Guardar_Click(object sender, EventArgs e)
        {
            ...
            EncabezadoIndustriaComercio Generico = new EncabezadoIndustriaComercio();
            ManejoInformacionMonitoreo Mim = new ManejoInformacionMonitoreo();           
            Mensajes_Error_BDD er = new Mensajes_Error_BDD();
            Medios DA;           
            Validar_Data v;
            ...
            Generico.Id_Tema = procesos.IntNULLCombo(CboTema);
            Generico.Tema_Especifico = txtema.Text.ToString();  
            Generico.Id_TipoRegistro = procesos.IntNULLCombo(cboTipoRegistro);
            Generico.NumeroRegistro = txtNumeroRegistro.Text.ToString();  
            Generico.Id_TipoOrganizacion = procesos.IntNULLCombo(CboTipoOrganizacion);
            Generico.Tema_Organizacion = txttemaOrganizacion.Text.ToString(); 
            Generico.Id_TipoEmpresa = procesos.IntNULLCombo(CboTipoEmpresa);
            Generico.Tema_Empresa = txttemaEmpresa.Text.ToString();
            Generico.Producto_Recomendado = txttipoproductoR.Text.ToString();
            ...
            v = Validar.Campos_Obligatorio_IndustriaComercio(Generico, RadDocumentoVerificacion, DMI,Mantenimiento);
            if (v.Verificar)
            {
                MensajePantalla(v.Mensaje);
            }
            else
            {
                ...
                XmlDocument DescripcionProducto = Mim.DetalleXMLProductoEncabezadoIndustriayComercio(Generico);

                if (Mim.GuardarItemEncabezado(6, DMI, DescripcionProducto, ref er))
                {
        ...
        protected void Eliminar_Items(object source, GridCommandEventArgs e)
        {
            ...
            {
                if (Mim.Eliminacion_Producto(8, edm, ref er))
                {
                    RecargarGrid();
                }
                else
        ...
        protected void Seleccionar_Items(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Delete") { Llave = 1; }
              ...
              if (Llave == 0)
              {
                ...
                {
                    x1.Visible = true;
                    lblTema.Text = item.GetDataKeyValue("DescripcionTema").ToString();
                    if(Convert.ToInt32(item.GetDataKeyValue("Id_Tema").ToString()) == 7) 
                    {
                        LblTemaEspecifico.Text = item.GetDataKeyValue("Tema_Especifico").ToString();
                        otro1.Visible = true; 
                    }
                    else 
                    {
                ...
                {
                    x2.Visible = true;
                    LblTipoRegistro.Text = item.GetDataKeyValue("TipoRegistro").ToString();
                    LblRegistro.Text = item.GetDataKeyValue("NumeroRegistro").ToString();                                                
                    LblTipoOrganizacion.Text = item.GetDataKeyValue("DescripcioOrganizacion").ToString();

                    if (Convert.ToInt32(item.GetDataKeyValue("Id_TipoOrganizacion").ToString()) == 4) 
                    {
                        LblTemaOrganizacion.Text = item.GetDataKeyValue("Tema_Organizacion").ToString();
                        otro2.Visible = true; 
                    }
                    else 
                    {
                    ...
                    LbltipoEmpresa.Text = item.GetDataKeyValue("DescripcionEmpresa").ToString(); 
                    if (Convert.ToInt32(item.GetDataKeyValue("Id_TipoEmpresa").ToString()) == 4)
                    {
                        LbTemaEmpresa.Text = item.GetDataKeyValue("Tema_Empresa").ToString();
                        otro3.Visible = true;
                    }
                    else
                    {
                    ...
                    LblProductoR.Text = item.GetDataKeyValue("Producto_Recomendado").ToString();                                        
                    valorA = "560";
                }
                OpenWinwdows(VerDatosExtra, "520", valorA , "Key", "Información Adicional del Ingresos");                
              }
        }
